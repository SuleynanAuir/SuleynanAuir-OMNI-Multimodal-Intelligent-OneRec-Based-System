...
class ReReTrainer(Trainer):
    ...
    def _normalize_sid_text(self, text):
            ...
            with unwrap_model_for_generation(self.model, self.accelerator) as unwrapped_model:
                ...
                if self.test_during_training:
                    ...
                    for i, comp_lis in enumerate(test_comp_lis):
                        target = self._normalize_sid_text(dedup_target[i])
                        for j in range(len(comp_lis)):
                            pred = self._normalize_sid_text(comp_lis[j])
                            if pred == target:
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        policy_term = torch.exp(per_token_logps - per_token_logps.detach()) * advantages.unsqueeze(1)
        per_token_loss = -(policy_term - self.beta * per_token_kl)

        seq_policy_loss = -((policy_term * completion_mask).sum(dim=1) / completion_mask.sum(dim=1))
        seq_kl = (per_token_kl * completion_mask).sum(dim=1) / completion_mask.sum(dim=1)
        seq_kl_loss = self.beta * seq_kl
        ...
        self._metrics["policy_loss"].append(self.accelerator.gather_for_metrics(seq_policy_loss.mean()).mean().item())
        self._metrics["kl_loss"].append(self.accelerator.gather_for_metrics(seq_kl_loss.mean()).mean().item())
        self._metrics["adv_mean"].append(self.accelerator.gather_for_metrics(advantages.mean()).mean().item())
        self._metrics["adv_abs_mean"].append(self.accelerator.gather_for_metrics(advantages.abs().mean()).mean().item())
        self._metrics["adv_std"].append(self.accelerator.gather_for_metrics(advantages.std()).mean().item())
        self._metrics["loss_raw"].append(self.accelerator.gather_for_metrics(loss.detach()).mean().item())
        self._metrics["loss_abs"].append(self.accelerator.gather_for_metrics(loss.detach().abs()).mean().item())
    ...
    def log(self, logs: dict[str, float], start_time: Optional[float] = None) -> None:
        ...
        logs = {**logs, **metrics}
        if "loss_raw" in metrics:
            logs["loss_display"] = metrics["loss_raw"]
            loss_val = logs.get("loss")
            if isinstance(loss_val, (int, float)) and abs(loss_val) < 1e-8:
                logs["loss"] = metrics["loss_raw"]
