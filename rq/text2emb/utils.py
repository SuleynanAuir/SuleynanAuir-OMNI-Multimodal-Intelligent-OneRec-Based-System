...
def _split_api_keys(raw_keys):
    if not raw_keys:
        return []
    return [key.strip() for key in raw_keys.split(',') if key.strip()]


def _resolve_api_info(api_info=None):
    is_explicit_api_info = api_info is not None
    api_info = dict(api_info or {})

    file_env = _load_dotenv_file('.env')

    provider = api_info.get('provider')
    if not provider:
        if is_explicit_api_info:
            provider = 'deepseek'
        else:
            provider = os.getenv('LLM_PROVIDER') or file_env.get('LLM_PROVIDER') or 'deepseek'
    provider = str(provider).strip().lower()

    merged_key_list = []
    merged_key_list.extend(api_info.get('api_key_list', []))
    merged_key_list.extend(_split_api_keys(os.getenv('API_KEY_LIST') or file_env.get('API_KEY_LIST', '')))

    provider_specific_key_names = {
        'minimax': 'MINIMAX_API_KEY',
        'deepseek': 'DEEPSEEK_API_KEY',
        'openai': 'OPENAI_API_KEY',
    }
    provider_key_name = provider_specific_key_names.get(provider, 'OPENAI_API_KEY')
    provider_specific_key = os.getenv(provider_key_name) or file_env.get(provider_key_name)
    if provider_specific_key:
        merged_key_list.append(provider_specific_key.strip())

    deduped_keys = []
    for key in merged_key_list:
        if key and key not in deduped_keys:
            deduped_keys.append(key)

    base_url = api_info.get('base_url')
    if not base_url:
        provider_specific_base_url_names = {
            'minimax': 'MINIMAX_BASE_URL',
            'deepseek': 'DEEPSEEK_BASE_URL',
            'openai': 'OPENAI_BASE_URL',
        }
        base_url_name = provider_specific_base_url_names.get(provider)
        if base_url_name:
            base_url = os.getenv(base_url_name) or file_env.get(base_url_name)

    resolved_api_info = dict(api_info)
    resolved_api_info['provider'] = provider
    resolved_api_info['api_key_list'] = deduped_keys
    if base_url:
        resolved_api_info['base_url'] = base_url

    if 'temperature' not in resolved_api_info:
        raw_temperature = os.getenv('TEMPERATURE') or file_env.get('TEMPERATURE')
        if raw_temperature:
            try:
                resolved_api_info['temperature'] = float(raw_temperature)
            except ValueError:
                pass

    return resolved_api_info



def get_res_batch(model_name, prompt_list, max_tokens, api_info=None):

    api_info = _resolve_api_info(api_info)

    provider = api_info.get("provider", "deepseek")
...
def get_openai_batch(model_name, prompt_list, max_tokens, api_info):
    if api_info.get("api_key_list"):
        openai.api_key = api_info["api_key_list"][-1]
