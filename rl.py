...
def train(
    ...
    def sid_similarity(pred_sid, target_sid):
        pred = normalize_sid(pred_sid)
        ...
        pred_parts = re.findall(r"\[[^\[\]]+\]", pred)
        target_parts = re.findall(r"\[[^\[\]]+\]", target)

        if not pred_parts or not target_parts:
            return 0.0

        max_len = max(len(pred_parts), len(target_parts))
        if max_len == 0:
            return 0.0
        ...
        position_match = sum(1 for pred_part, target_part in zip(pred_parts, target_parts) if pred_part == target_part)

        return 0.7 * (prefix_match / max_len) + 0.3 * (position_match / max_len)

    def fallback_text_similarity(pred_sid, target_sid):
        pred = normalize_sid(pred_sid)
        target = normalize_sid(target_sid)
        if not pred or not target:
            return 0.0
        ...
        if not pred_compact or not target_compact:
            return 0.0
        return float(SequenceMatcher(None, pred_compact, target_compact).ratio())

    def shaped_similarity(pred_sid, target_sid):
        pred_norm = normalize_sid(pred_sid)
        target_norm = normalize_sid(target_sid)

        if pred_norm == target_norm and pred_norm:
            return 1.0

        sid_score = sid_similarity(pred_norm, target_norm)
        txt_score = fallback_text_similarity(pred_norm, target_norm)
        has_structured_sid = bool(re.findall(r"\[[^\[\]]+\]", pred_norm))
        format_bonus = 0.05 if has_structured_sid else 0.0

        score = 0.65 * sid_score + 0.35 * txt_score + format_bonus
    ...
    def ndcg_rule_reward(prompts, completions):
        ...
        for i, completion in enumerate(completions):
            target = targets[i]
            pred_norm = normalize_sid(completion)
            target_norm = normalize_sid(target)
            rank = i % num_generations

            if pred_norm == target_norm:
                base_reward = 1.0
            else:
                base_reward = 0.25 * shaped_similarity(pred_norm, target_norm)

            shaped_scores.append(base_reward)

            rewards.append(base_reward / math.log2(rank + 2))

        if reward_debug_state["calls"] < 2:
            print(
                f"[RL Reward Debug] ndcg_rule_reward call={reward_debug_state['calls']} "
                f"avg={float(np.mean(shaped_scores)):.4f} max={float(np.max(shaped_scores)):.4f}"
            )
    ...
    def rule_reward(prompts, completions):
        ...
        for i, completion in enumerate(completions):
            ...
            if pred_norm == target_norm:
                ...
                sim = shaped_similarity(pred_norm, target_norm)
            sims.append(sim)
            rewards.append(sim)

        if reward_debug_state["calls"] < 2:
            print(
                f"[RL Reward Debug] rule_reward call={reward_debug_state['calls']} "
                f"avg={float(np.mean(sims)):.4f} max={float(np.max(sims)):.4f}"
            )
            for idx in range(min(2, len(completions))):
                print(
                    "[RL Reward Debug] sample", idx,
                    "pred=", normalize_sid(completions[idx])[:120],
                    "target=", normalize_sid(targets[idx])[:120],
                )
        reward_debug_state["calls"] += 1
        return rewards
